...
export class PaymentService {
    ...
    constructor(private prisma: PrismaService) {
        // Initialize Moyasar client with API key
        this.moyasarClient = moyasar.default;
        moyasar.default.setApiKey(process.env.MOYASAR_SECRET_KEY || '');
    }
    ...
    async handleWebhook(payload: any) {
        ...
        if (type === 'payment_paid') {
            ...
            const payment = await this.prisma.payment.findFirst({
                where: { moyasarPaymentId: data.id },
            });
    ...
    async refundPayment(userId: string, paymentId: string, reason?: string) {
        ...
        try {
            ...
            const refund = await this.moyasarClient.Refund.create({
                payment_id: payment.moyasarPaymentId,
            ...
            await this.prisma.payment.update({
                ...
                data: {
                    ...
                    refundedAt: new Date(),
                    moyasarResponse: { ...payment.moyasarResponse, refund },
                },
            });
