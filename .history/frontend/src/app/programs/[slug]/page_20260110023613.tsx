...
export default function ProgramDetailPage() {
    ...
    const handleRegister = (cohortId?: string) => {
        if (!isAuthenticated) {
            router.push(`/login?redirect=/programs/${slug}`);
            return;
        }
        
        const checkoutUrl = cohortId 
            ? `/checkout?program=${slug}&cohort=${cohortId}`
            : `/checkout?program=${slug}`;
    ...
    // Old mock data structure - keeping for reference but not using
