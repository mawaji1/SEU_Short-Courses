...
export class TabbyService {
  ...
  async getInstallmentOptions(
    ...
    try {
      ...
      this.logger.error(
        'Failed to get Tabby installment options:',
  ...
  async createCheckout(
    ...
    try {
      ...
      this.logger.log(
        `Tabby checkout session created: ${sessionData.id}, status: ${sessionStatus}`,
      );
      ...
      const installmentProducts =
        sessionData.configuration?.available_products?.installments || [];
      ...
      this.logger.error(
        'Tabby checkout creation failed:',
        error.response?.data || error.message,
      );
      return {
        success: false,
  ...
  async getPaymentStatus(paymentId: string): Promise<any> {
    try {
      ...
      this.logger.log(
        `Tabby payment status: ${paymentId} - ${response.data.status}`,
      );
      return response.data;
    } catch (error) {
      this.logger.error(
        `Failed to get Tabby payment status for ${paymentId}:`,
        error.response?.data || error.message,
      );
  ...
  async checkEligibility(
    ...
    try {
      ...
      if (status === 'created') {
        ...
        const reason =
          response.data.configuration?.products?.installments
      ...
      this.logger.error(
        'Tabby eligibility check failed:',
        error.response?.data || error.message,
      );
