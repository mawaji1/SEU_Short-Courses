...
export class TamaraWebhookService {
     ...
     */
    async processWebhook(payload: any, tamaraToken?: string, authHeader?: string): Promise<void> {
        const { order_id, order_reference_id, order_status, data } = payload;

        this.logger.log(`Processing Tamara webhook: ${order_id}, status: ${order_status}`);
        ...
        if (tamaraToken) {
            ...
            if (!isValid) {
                this.logger.warn(`Invalid Tamara token in auth header for order ${order_id}`);
                return;
            }
        }
    ...
    private async handleApproved(payment: any, payload: any): Promise<void> {
        ...
        try {
            ...
            const authorizeResult = await this.tamaraService.authorizeOrder(tamaraOrderId);
            ...
            this.logger.error(`Failed to process approved webhook for ${paymentId}:`, error);
            throw error;
        }
    }
