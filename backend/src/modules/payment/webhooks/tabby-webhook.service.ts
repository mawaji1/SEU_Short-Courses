...
export class TabbyWebhookService {
    ...
    async processWebhook(payload: any, authHeader?: string): Promise<void> {
        ...
        this.logger.log(`Processing Tabby webhook: ${paymentId}, status: ${status}`);
    ...
    private async handleAuthorized(payment: any, payload: any): Promise<void> {
        ...
        try {
            ...
            const verifiedPayment = await this.tabbyService.getPaymentStatus(tabbyPaymentId);

            if (verifiedPayment.status !== 'AUTHORIZED') {
                this.logger.error(`Payment status mismatch: expected AUTHORIZED, got ${verifiedPayment.status}`);
                return;
            }
            ...
            if (Math.abs(expectedAmount - actualAmount) > 0.01) {
                this.logger.error(`Amount mismatch: expected ${expectedAmount}, got ${actualAmount}`);
                return;
            }
            ...
            try {
                ...
                this.logger.error(`Failed to capture payment ${paymentId}:`, captureError);
            ...
            this.logger.error(`Failed to process authorized webhook for ${paymentId}:`, error);
            throw error;
        }
    }
