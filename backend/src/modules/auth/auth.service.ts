...
export class AuthService {
    ...
    async forgotPassword(email: string): Promise<{ message: string }> {
        ...
        if (!user) {
            return { message: 'إذا كان البريد الإلكتروني مسجلاً، ستصلك رسالة بإرشادات إعادة تعيين كلمة المرور' };
        }
     ...
     */
    private generateAuthResponse(user: { id: string; email: string; firstName: string; lastName: string; role: string }): AuthResponse {
        const payload = {
            sub: user.id,
            email: user.email,
            role: user.role,
