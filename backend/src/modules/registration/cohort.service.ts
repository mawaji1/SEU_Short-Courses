...
export class CohortService {
     ...
     */
    async findAllCohorts(programId?: string, status?: CohortStatus): Promise<CohortResponseDto[]> {
        const where: any = {};
    ...
    async findUpcomingCohorts(programId: string): Promise<CohortResponseDto[]> {
        const cohorts = await this.prisma.cohort.findMany({
            where: {
                programId,
                status: { in: [CohortStatus.UPCOMING, CohortStatus.OPEN, CohortStatus.FULL] },
